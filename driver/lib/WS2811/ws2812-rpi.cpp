...
void NeoPixel::show(){
    ...
    LEDBufferCopy=LEDBuffer;

    for(i=0; i<numLEDs; i++) {
        LEDBufferCopy[i].r *= brightness;
        LEDBufferCopy[i].g *= brightness;
        LEDBufferCopy[i].b *= brightness;
        colorBits = ((unsigned int)LEDBufferCopy[i].r << 8) | ((unsigned int)LEDBufferCopy[i].g << 16) | LEDBufferCopy[i].b;
...
void NeoPixel::clearLEDBuffer(unsigned int i) {
    LEDBuffer[i].r = 0;
    LEDBuffer[i].g = 0;
    LEDBuffer[i].b = 0;
}
